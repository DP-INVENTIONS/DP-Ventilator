...
#define  pin_Strobe_TM         53
#define  pin_Clock_TM          51
#define  pin_DIO_TM            49
#endif
...
void updateBargraph()
{
 if (ee.CVmode)  // CV Mode - bargraph needs to show compression
  ...
  {           // CP Mode - bargraph needs to show volume 
   ...
   /*+++ NEXTION dvdh +++*/
   #ifdef NxtDisplay
    float vol=stepper.currentPosition()/(motorVolumeRatio*maxVolume); 
    int c=int (8*vol); 
    barGraph=0;
    if(c<1) c=1; // Always make sure at least the downmost position is active
 ...
 if (timerLvl>1)  // only does the motor update if nested more than 1 level
  ...
  else
  {
   ...
   Watchdog.reset();
    ...
    /*+++ NEXTION dvdh +++*/
    #ifdef NxtDisplay
         NxtBargraph = (breathGraphCnt>0)?(barGraph)-bargraphDot:barGraph;
...
strip.show();
#else
pinMode(pin_LED, OUTPUT); // warning led pinout at OUTPUT
#endif
...
//    stepper.run();
  ...
  if (LedOnOff == LOW)
    {
      if (alarmNum>0) strip.setPixelColor(0, strip.Color(20,0,0));
      else strip.setPixelColor(0, strip.Color(0,20,0));
...
#else
      /*+++ NEXTION dvdh +++*/
      digitalWrite(pin_Stepper_Disable, LOW);  // Always activated
      motorState=HIGH; 
#endif     
   }
